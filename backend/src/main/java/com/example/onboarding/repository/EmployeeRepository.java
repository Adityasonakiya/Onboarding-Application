...
List<EmployeeCandidateDTO> searchEmployeeByClientName(@Param("hsbchiringManager") String hsbchiringManager);
    ...
    @Query(value = "SELECT e.psid AS psid, r.role_name AS roleName, lob.lob_name AS lobName, "
            ...
            + "CASE WHEN r.role_name IN ('Admin', 'Onboarding Team', 'DM','LOB-PMO','Sales SPOC') THEN 1 ELSE 0 END AS canViewLandingPage, "
            + "CASE WHEN r.role_name IN ('Admin', 'Onboarding Team') THEN 1 ELSE 0 END AS canAccessAdminDashboard, "
        //     + "CASE WHEN r.role_name IN ('LOB-PMO') AND selection.lob_id = lob.lob_id THEN 1 ELSE 0 END AS canAccessLOB, "
