...
public interface SelectionDetailsRepository extends JpaRepository<SelectionDetails, Integer> {
    ...
    @Query(value = "(SELECT obs.onboarding_status AS onboardingStatus, emp.psid AS ltiPsId, emp.first_name AS firstName, emp.last_name AS lastName, "
            ...
            + "selection.base_bu AS baseBu, lob.lob_name as lobName, sublob.sub_lob_name as subLobName, lob.salespoc AS salesPoc, selection.hsbchiring_manager AS hsbcHiringManager, "
            + "lob.hsbchead AS hsbcHead, lob.delivery_manager AS deliveryManager, selection.irm AS irm, selection.pricing_model AS pricingModel, "
