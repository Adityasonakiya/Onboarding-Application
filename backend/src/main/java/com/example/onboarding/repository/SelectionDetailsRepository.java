...
public interface SelectionDetailsRepository extends JpaRepository<SelectionDetails, Integer> {
    ...
    @Query(value = "(SELECT obs.onboarding_status AS onboardingStatus, emp.psid AS ltiPsId, emp.first_name AS firstName, emp.last_name AS lastName, "
            ...
            + "td.status_remarks AS remarks, CASE WHEN evd.selection_id IS NOT NULL THEN 'Yes' ELSE 'No' END AS interviewDocuments, CASE WHEN selection.dojreceived_date IS NOT NULL THEN Date(selection.dojreceived_date) ELSE NULL END AS hsbcConfirmedDoj,"
            + "CASE WHEN DATEDIFF(selection.dojreceived_date, selection.hsbcselection_date)>=0 THEN DATEDIFF(selection.dojreceived_date, selection.hsbcselection_date) ELSE 0 END AS agingSelectionWithDoj, "
            + "CASE WHEN DATEDIFF(selection.dojreceived_date, selection.hsbcselection_date) IS NOT NULL THEN FLOOR(DATEDIFF(selection.dojreceived_date, selection.hsbcselection_date) / 7) ELSE 0 END AS hsbcDojAgingBucket, "
