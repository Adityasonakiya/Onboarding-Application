...
public interface SelectionDetailsRepository extends JpaRepository<SelectionDetails, Integer> {
    ...
    Integer countByLob_LobId(int lobId);
    ...
    Page<EmployeeCandidateDTO> findEmployeeCandidates(@Param("loggedInPsid") Integer loggedInPsid, Pageable pageable);
    ...
    @Query(value = "(SELECT obs.onboarding_status AS onboardingStatus,selection.hsbc_id as hsbcId, emp.psid AS ltiPsId, emp.first_name AS firstName, emp.last_name AS lastName, "
            + "emp.grade AS grade, emp.location AS location, CASE WHEN emp.total_experience < 1 THEN CONCAT(FLOOR(emp.total_experience * 12), ' months') ELSE CONCAT(emp.total_experience, ' years') END AS totalExperience, emp.skill AS skill, "
            ...
            + "Date(selection.ctool_start_date) AS ctoolStartDate, selection.recruiter_name AS recruiterName, selection.ctool_tagging_rate AS ctoolTaggingRate, selection.ctool_proposed_rate AS ctoolProposedRate, "
            ...
            + "td.status_remarks AS remarks, CASE WHEN evd.selection_id IS NOT NULL THEN 'Yes' ELSE 'No' END AS interviewDocuments, CASE WHEN selection.hsbconboarding_date IS NOT NULL THEN Date(selection.hsbconboarding_date) ELSE NULL END AS hsbcConfirmedDoj,"
            + "CASE WHEN DATEDIFF(selection.hsbconboarding_date, selection.hsbcselection_date)>=0 THEN DATEDIFF(selection.hsbconboarding_date, selection.hsbcselection_date) ELSE 0 END AS agingSelectionWithDoj, "
            + "CASE WHEN DATEDIFF(selection.hsbconboarding_date, selection.hsbcselection_date) IS NOT NULL THEN FLOOR(DATEDIFF(selection.hsbconboarding_date, selection.hsbcselection_date) / 7) ELSE 0 END AS hsbcDojAgingBucket, "
            + "Date(selection.billing_start_date) AS billingStartDate, Date(td.create_date) AS taggingDone, "
            + "Date(selection.tech_selection_date) AS techSelectionDone, CASE WHEN selection.dojreceived_date IS NOT NULL THEN Date(selection.dojreceived_date) ELSE NULL END AS dojRecievedDate "
            + "FROM  employee emp "
